...
const toolDefinitions = [
  {
    ...
    description:
      'يتحقق من الأيام المتاحة للحجز في العيادة. الحجز صار باليوم (مو بالساعة) — كل يوم له عدد محدد من المواعيد أو مفتوح. استخدمه عندما يريد المريض معرفة الأيام المتاحة أو يريد يحجز.',
    input_schema: {
      type: 'object',
      properties: {
        date_preference: {
          type: 'string',
          description:
            'تفضيل المريض للتاريخ: "اليوم"، "غداً"، "الأحد"، "الأسبوع القادم"، أو تاريخ محدد بصيغة YYYY-MM-DD',
        },
      },
  ...
  },
  {
    ...
    description:
      'يحجز موعداً للمريض في يوم محدد ويعطيه رقم بالدور تلقائياً. استخدمه بعد الحصول على اسم المريض واليوم المطلوب. لا يحتاج ساعة محددة.',
    input_schema: {
      type: 'object',
      properties: {
        patient_name: {
          type: 'string',
          description: 'الاسم الكامل للمريض كما ذكره في هذه المحادثة',
        },
        appointment_date: {
          type: 'string',
          description:
            'اليوم المطلوب للحجز: تفضيل مثل "غداً"، "الأحد"، أو تاريخ بصيغة YYYY-MM-DD (يفضّل استخدام حقل date من نتائج check_availability)',
        },
        reason: {
          type: 'string',
          description: 'سبب الزيارة أو الشكوى الرئيسية التي ذكرها المريض',
        },
      },
  ...
  },
  {
    ...
    description:
      'يرجع عدد الحجوزات (المواعيد المحجوزة) في يوم معين. استخدمه عندما يسأل المريض كم حجز موجود اليوم أو في يوم معين.',
    input_schema: {
      type: 'object',
      properties: {
        date_preference: {
          type: 'string',
          description: 'اليوم المسؤول عنه: "اليوم"، "غداً"، "الأحد"، أو تاريخ YYYY-MM-DD',
        },
      },
      required: ['date_preference'],
    },
  },
  {
    ...
    description:
      'يلغي الموعد القادم للمريض بناءً على رقم هاتفه.',
    input_schema: {
      type: 'object',
      properties: {
        patient_phone: {
          type: 'string',
          description: 'رقم هاتف المريض (E.164)',
        },
      },
  ...
  },
  {
    name: 'get_faq_answer',
    description:
      'يبحث عن إجابة جاهزة في قاعدة الأسئلة الشائعة للعيادة. استخدمه للأسئلة عن الخدمات والأسعار والأوقات وما شابه.',
    input_schema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'السؤال الذي طرحه المريض كما هو',
        },
      },
      required: ['question'],
    },
  },
  {
    ...
    description:
      'ينقل المحادثة إلى موظف بشري. استخدمه فقط عند وجود مشكلة حقيقية أو طلب خارج نطاق الصلاحيات.',
    input_schema: {
      type: 'object',
      properties: {
        reason: {
          type: 'string',
          description: 'سبب التصعيد باختصار',
        },
      },
      required: ['reason'],
    },
...
async function executeTool(name, input, context) {
  switch (name) {
    ...
    case 'get_faq_answer':      return getFaqAnswer(input, context);
...
async function getFaqAnswer({ question }, { clinic }) {
  try {
    ...
    logger.error('getFaqAnswer error', { error: err.message });
